...
if($_SERVER['REQUEST_METHOD'] === "POST")
{
    ...
    if(isset($data->title) && isset($data->description) && isset($data->genre) && isset($data->rating) && isset($data->content_rating) && isset($data->release_date) && (isset($data->duration) || isset($data->seasons)) && isset($data->crew) && isset($data->type))
    {
        ...
        if($data->type !== "movie" && $data->type !== "tvshow")
        {
            echo json_encode(array("status" => "error", "timestamp" => time(), "data" => "Type must be either movie or tvshow"));
            header("HTTP/1.1 400 Bad Request");
            return;
        }
        if($data->type === "movie" && isset($data->seasons))
        {
            echo json_encode(array("status" => "error", "timestamp" => time(), "data" => "A Movie should not have a number of seasons"));
            header("HTTP/1.1 400 Bad Request");
            return;
        }
        if($data->type === "tvshow" && isset($data->duration))
        {
            echo json_encode(array("status" => "error", "timestamp" => time(), "data" => "A Movie should have a number of seasons instead of duration"));
            header("HTTP/1.1 400 Bad Request");
            return;
        }
        ...
        if($data->type === "tvshow" && !isset($data->seasons))
        {
            echo json_encode(array("status" => "error", "timestamp" => time(), "data" => "tvshow should have a number of seasons"));
            header("HTTP/1.1 400 Bad Request");
            return;
        }
